"""
Baseline 3: 1D-CNN over raw 300x8 sequences, GroupKFold(n_splits=5) by user.

Drop-in companion to baseline_lgbm.py. Uses the SAME fold_assignments.csv so
CV scores are directly comparable.

Input format per file: a (300, 6) CSV with columns mean_x/y/z, std_x/y/z.
We construct an 8-channel sequence:
  - 6 raw channels, per-file normalized (zero mean, unit std on that file)
  - 2 magnitude channels (||mean||, ||std||), NOT normalized — orientation-invariant

Architecture: 3 conv blocks (Conv1d -> BN -> ReLU -> MaxPool) + global avg pool + FC.
~50k params. Trained with class-balanced cross-entropy, AdamW, cosine schedule,
...
Outputs (matching baseline_lgbm.py schema):
    oof_predictions_cnn.csv
    test_proba_cnn.csv
    submission_cnn.csv
    baseline_cnn_log.txt
"""
import argparse
import time
...
def main():
    ...
    oof_df.to_csv(out_dir / "oof_predictions_cnn.csv", index=False)
    log(f"\nWrote {out_dir/'oof_predictions_cnn.csv'}")

    # ---- save test predictions and submission ----
    test_pred = test_proba.argmax(axis=1)
    sub = pd.DataFrame({"Id": test_idx["file_id"].values, "Label": test_pred})
    sub = sub.sort_values("Id").reset_index(drop=True)
    sub.to_csv(out_dir / "submission_cnn.csv", index=False)
    log(f"Wrote {out_dir/'submission_cnn.csv'}  ({len(sub)} rows)")
    ...
    test_proba_df.to_csv(out_dir / "test_proba_cnn.csv", index=False)
    log(f"Wrote {out_dir/'test_proba_cnn.csv'}")

    (out_dir / "baseline_cnn_log.txt").write_text("\n".join(log_lines))
    log(f"Wrote {out_dir/'baseline_cnn_log.txt'}")
